...
class ChatProvider with ChangeNotifier {
  ...
  unMatch(Chat chat) async {
    try {
      ...
      final unMatch = await dio.post(Endpoints.unMatch,
  ...
  //update last message
  Future<void> updateLastMsg(String msg, String chatId, DateTime time,
      {bool? isBlocked, String? targetMsg, String? curreUsMsg}) async {
    final int currentUserId = Hive.box('user').get('id');
    await usersRef
        .doc(currentUserId.toString())
        .collection('chats')
  ...
  initiateChat(int userId, int matchId) async {
    try {
      ...
      final response = await dio.post(Endpoints.initiateChat,
  ...
  void sendNotification(
      String name, String msg, String pushId, Chat chat, chatId) {
    final dio = Dio();
